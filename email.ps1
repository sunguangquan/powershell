...
#发送邮件
if(($cc.length -gt 0) -and ($fujian.Length -gt 0)){
...
elseif($fujian.length -gt 0){
    Send-MailMessage -From $from -To $to -Attachments $fujian -Body $body -Subject $Subject -SmtpServer $SmtpServer -Credential $cred -Encoding "UTF8"  -BodyAsHtml
}
elseif($cc.Length -gt 0){
    Send-MailMessage -From $from -To $to  -Cc $cc  -Body $body -Subject $Subject -SmtpServer $SmtpServer -Credential $cred -Encoding "UTF8"  -BodyAsHtml
}
else{
    Send-MailMessage -From $from -To $to  -Body $body -Subject $Subject -SmtpServer $SmtpServer -Credential $cred -Encoding "UTF8"  -BodyAsHtml
}
}